...
import com.waji.local.DeleteFile;
...
public class QiandaoActivity extends Activity {
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				break;
			case 0:
				Toast.makeText(getApplicationContext(), "签到失败，请检查网络是否连接", 1).show();
				 finish();
				break;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		file.Delete(getApplicationContext());
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					String result = send.send(getApplicationContext(),
							"-1");
					msg.what = 1;
					msg.obj = result;
	...
	public void Toreturn(View view){
		try {
			SharedPrefer.SaveLeaveStateContext(getApplicationContext(), "leave");
		} catch (Exception e) {
			 e.printStackTrace();
		}
		finish();
